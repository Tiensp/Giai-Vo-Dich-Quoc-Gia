...
namespace QLBDUI.GiaiDauFD.QLDB
{
    public partial class themMoiDB : Form
    {
        ...
        private ThamSoDTO thamso = new ThamSoDTO();
        private List<CauThuDTO> ListCauThuSTO = new List<CauThuDTO>();
        private ThemCT formthemCT = new ThemCT(0);
        ...
        {
            ///
            thamso = tsBUS.getData("1");
            if (thamso == null) thamso = tsBUS.getData("0");
            ...
            if(check)
            {
                ...
                if(result == DialogResult.Yes)//Nếu đội bóng đã tồn tại và muốn thay đổi thông tin đội bóng đó.
                {
                    ...
                    foreach (CauThuDTO ct in listct)
                    {
                        ...
                        if(!ktdata)
                            if (ct.MaDoiBong == textBox3.Text)//xét cầu thủ thuộc đội bóng này.
                            {
                    ...
                    if (result2 == DialogResult.Yes)
                    {
                        ...
                        {
                            if (ct.MaLoaiCT == "1   ") socauthungoai++;
                        }
                    }
                    ...
                    else if(dbDTO.SoCauThuNgoai > thamso.SoCTNgoaiMax)
                    {
                        MessageBox.Show("Số lượng cầu thủ ngoại vượt quá cho phép!", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                        check = false;
                    }
                    else if(dbDTO.SoLuongCauThu < thamso.SoLuongCTMin)
                    {
                        MessageBox.Show("Số lượng cầu thủ ít hơn số lượng cho phép!", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                        check = false;
                    }
                    else if (dbDTO.SoLuongCauThu > thamso.SoLuongCTMax)
                    {
                        MessageBox.Show("Số lượng cầu thủ nhiều hơn số lượng cho phép!", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                        check = false;
                    }
                    ...
                    if (check)
                    {
                        ...
                        if (kt == false)
                        ...
                        else
                            {
                                ...
                                if (result2 == DialogResult.No) this.Close();
                            }
        ...
        {
            List<DoiBongDTO> list = dbBUS.load();
            if (list == null) textBox3.Text = "0";
            else
                textBox3.Text = (int.Parse(list[list.Count-1].MaDoiBong) + 1).ToString();
            load_data_ct();
        }
        ...
        private void formthemCT_FormClosed(object sender, FormClosedEventArgs e)
        {
            ...
            int count;
            if (ListCauThuSTO == null) count = 0;
            else count = int.Parse(ListCauThuSTO[ListCauThuSTO.Count-1].MaCauThu) + 1;
            formthemCT = new ThemCT(count);
            this.Show();
            load_data_ct();
        }
        ...
        private void UpdateCTButt_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataGridViewRow item in this.dataGridView1.SelectedRows)
            {
                ...
                string loaict = item.Cells["LoaiCauThu"].Value.ToString();
        ...
        private void formcapnhat_FormClosed(object sender, FormClosedEventArgs e)
        {
            if (formcapnhat.send_data() != null)
            {
                foreach (DataGridViewRow item in this.dataGridView1.SelectedRows)
                {
                    for (int i = 0; i < ListCauThuSTO.Count; i++)
                    {
                        if (ListCauThuSTO[i].MaCauThu == item.Cells["Ma"].Value.ToString())
                        {
            ...
            formthemCT = new ThemCT(ListCauThuSTO.Count);
            this.Show();
            load_data_ct();
        }
                ...
                for (int i = 0; i < ListCauThuSTO.Count; i++)
                {
                    if (ListCauThuSTO[i].MaCauThu == item.Cells["Ma"].Value.ToString())
                    {
                        if (textBox4.Text != null)
                        {
                            ...
                            foreach (CauThuDTO ct in listct)
                            {
                                if (ct.MaCauThu == item.Cells["Ma"].Value.ToString())
                                {
